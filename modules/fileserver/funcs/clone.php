...
if ($rank > 0) {
    $lev = $rank;
    $base_url .= '&rank=' . $rank;
    $selected_folder_path = $db->query('SELECT file_name FROM ' . NV_PREFIXLANG . '_' . $module_data . '_files WHERE file_id = ' . $rank)
        ->fetchColumn() ?: '';
} elseif ($root == 1) {
    $lev = 0;
    $base_url .= '&root=1';
                    ...
                    if (rename($full_path, NV_ROOTDIR . $new_file_path)) {
                        ...
                        try {
                            ...
                            if ($check_fileid > 0) {
                                ...
                                SET file_path = :file_path, file_size = :file_size, elastic = 0, updated_at = :updated_at 
